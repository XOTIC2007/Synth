    ...
    try {
      ...
      if (!urlRegex.test(query)) {
        let msgssa = await message.channel.send({
          content: `- Searching Songs on Synth Please Wait!`,
        });
...
function drawTitle(ctx, canvas) {
  ...
  ctx.fillText("Synth Music System", canvas.width / 2, 50);
  ctx.font = "18px Arial";
  ctx.fillStyle = "#cccccc";
  ctx.fillText("Synth Music - The Perfect Music Partner", canvas.width / 2, 80);
  ctx.font = "24px Arial";
  ctx.textAlign = "left";
}
