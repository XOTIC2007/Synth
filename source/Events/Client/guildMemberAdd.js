...
module.exports = async (client, member) => {
  ...
  // === Welcomer logic (inactive due to early return) ===
  ...
  try {
    ...
    await member
      .send({ embeds: [embed], components: [row], content: `[Synth Music](${client.config.support})` })
      .then(() => console.log(`Sent welcome message to ${member.user.tag}.`))
      .catch(() => {
        console.log(`Failed to send welcome message to ${member.user.tag}; they may have DMs disabled.`);
      });
  } catch (error) {
    console.error(`Error in guildMemberAdd for user ${member.user.tag}:`, error);
  }
};
