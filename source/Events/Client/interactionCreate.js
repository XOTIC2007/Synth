      ...
      try {
        ...
        try {
          ...
          const em = new EmbedBuilder()
            .setColor(client.config.color)
            .setTitle(`<a:music_01:1366780842411819058> Synth Music Invite`)
            .setURL(`https://discord.gg/dark-moon`)
            .setThumbnail(player.queue.current.thumbnail)
            .setDescription(
              `<a:gojo_nachra_hai:1363761986466222130> **${interaction.user.globalName}** has invited you to join **${memberVC.name}** Music Party in **${interaction.guild.name}**\n\nPlaying :  [${player.queue.current.title}](${channellink}) by ${player.queue.current.author}`
            )
            .addFields(
