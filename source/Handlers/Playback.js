...
module.exports = class Music {
  ...
  async handleError(ctx, err) {
    if (ctx.type === InteractionType.ApplicationCommand) {
      await ctx.editReply({
        content:
          "- An error occurred while playing the song. Try Reporting it to the Developer [Xotic](<https://discord.gg/dark-moon>)"
      });
      return this.console.error(err + " " + err.stack);
    } else {
      await ctx.reply({
        content:
          "- An error occurred while playing the song. Try Reporting it to the Developer [Itachi](<https://discord.gg/dark-moon>)"
      });
      return this.console.error(err + " " + err.stack);
    }
  }
  ...
  async Skip(ctx) {
    try {
      ...
      player.skip();
      const newMsg = `<a:tickmarkanimated:1377184982413344858> **${this.Name(
        ctx
  ...
  async Disconnect(ctx) {
    try {
      ...
      // }
      const newMsg = `<a:tickmarkanimated:1377184982413344858> **Disconnected** From Voice Channel!`;
      await this.ctxSend(ctx, { content: newMsg });
    } catch (err) {
      this.console.error(err);
    }
  ...
  async Queue(ctx, p) {
    try {
      ...
      if (player && player.queue.size == 0 && player.playing) {
        // const msg = `## <a:p_stars:1364625402949009489> Queue Of ${
        //   ctx.guild.name
  ...
  \`\`\``;
      ...
      const QueuePage = `## <a:p_stars:1364625402949009489> Queue Of ${
        ctx.guild.name
      }\n${
      ...
      collector.on("collect", async (i) => {
        ...
        if (i.customId === "previous_q") {
          ...
          const QueuePage = `## <a:p_stars:1364625402949009489> Queue Of ${ctx.guild.name}\n${pageMsg}\n\n**Page ${page}/${totalPages}**`;
